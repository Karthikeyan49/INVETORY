...
{
    /** Only the extended (tax) login may see / set the off-books extra amount. */
    ...
    public function index(Request $request): void
    {
        ...
        if (($st = $request->query('sale_type')) && in_array($st, ['cash', 'credit'], true)) {
            $where[]  = 'i.sale_type = ?';
            $params[] = $st;
        }
        ...
        $rows = Database::fetchAll(
            "SELECT i.invoice_id, i.invoice_number, i.order_id,
                    ...
                    COALESCE(i.sale_type, 'cash') AS sale_type,
        ...
        foreach ($rows as &$r) {
            ...
            // Off-books extra is visible only to the extended (tax) login.
            $r['extra_amount'] = $extended ? (float)$r['extra_amount'] : 0.0;
    ...
    public static function generateForOrder(int $orderId): ?array
    {
        ...
        if ($existing) {
            ...
            Database::execute(
                'UPDATE invoices SET status = ?, payment_method = ?, updated_at = NOW()
                 WHERE invoice_id = ?',
                [$newStatus, $order['payment_method'], $existing['invoice_id']]
            );
        ...
        $prefix    = Database::fetch("SELECT setting_value FROM settings WHERE setting_key = 'invoice_prefix'")['setting_value'] ?? 'INV';
        $maxId     = (int)(Database::fetch('SELECT MAX(invoice_id) AS max_id FROM invoices')['max_id'] ?? 0);
        ...
        $invoiceId = Database::insert(
            'INSERT INTO invoices (invoice_number, order_id, subtotal, gst_rate, gst_amount, delivery_fee, total, status, payment_method, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())',
            [
    ...
    public function show(Request $request): void
    {
        ...
        // Off-books extra is visible only to the extended (tax) login.
        $inv['extra_amount'] = self::isExtended($request) ? (float)($inv['extra_amount'] ?? 0) : 0.0;
    ...
    public function storeGst(Request $request): void
    {
        ...
        try {
            // Off-books extra (auto-carried from a machine) — only the extended login may set it.
            $extraAmount = self::isExtended($request) ? max(0.0, (float)($request->input('extra_amount') ?? 0)) : 0.0;

            // Sales module: cash = paid in full; credit = advance received, rest outstanding.
            $saleType = in_array($request->input('sale_type'), ['cash', 'credit'], true) ? (string)$request->input('sale_type') : 'cash';
            $advance  = max(0.0, (float)($request->input('advance') ?? 0));
            $location = $request->input('location') ? Request::sanitize((string)$request->input('location')) : null;
            $amountPaid = $saleType === 'cash' ? $total : min($advance, $total);
            $payStatus  = $amountPaid <= 0.005 ? 'unpaid' : ($amountPaid >= $total - 0.005 ? 'paid' : 'partial');
            $invStatus  = $payStatus === 'paid' ? 'Paid' : ($request->input('status') ?: 'Draft');

            $invoiceId = Database::insert(
                'INSERT INTO invoices
                    (invoice_number, order_id, customer_name, customer_gstin, customer_state,
                     customer_address, customer_city, location, customer_pincode, customer_country,
                     seller_state, due_date, subtotal, gst_rate, gst_amount, cgst_amount, sgst_amount, igst_amount,
                     delivery_fee, discount, total, extra_amount, sale_type, advance, amount_paid,
            ...
            // ── Mirror this sale into the orders ledger and link it back ────────
            // Revenue is booked net of GST (matching portal orders). A cash sale
            // (or fully-advanced credit) is paid → its order is paid → counts as
            // revenue now; otherwise Payment::refreshInvoice() flips it later.
            $orderId = Order::createDirect(
                Request::sanitize((string)$request->input('customer_name')),
                $taxable,
                [
                    ...
                    'source'         => 'invoice',
                    'notes'          => 'Auto-created from invoice ' . $invNumber,
                ]
            );
            Database::execute('UPDATE invoices SET order_id = ? WHERE invoice_id = ?', [$orderId, $invoiceId]);
    ...
    public function update(Request $request): void
    {
        ...
        // Off-books extra — only the extended (tax) login may change it.
        if (self::isExtended($request) && $request->input('extra_amount') !== null) {
            $sets[]   = 'extra_amount = ?';
            $params[] = max(0.0, (float)$request->input('extra_amount'));
        }
    ...
    public function download(Request $request): void
    {
        ...
        $lines   = [];
        $lines[] = str_repeat('=', 60);
        $lines[] = strtoupper($settings['company_name'] ?? 'INVENTORY MANAGEMENT SYSTEM');
        ...
        $lines[] = 'Thank you for choosing Inventory Management System!';
        $lines[] = str_repeat('=', 60);
