...
class AdminEmployeeController
{
    ...
    // ─── GET /admin/employees/{id}/inventory-consumption ────────────────────
    // Smart Inventory interconnection — items issued to this employee
    // (inventory_stock_movements where movement_type='EMPLOYEE_ISSUE' and
    // reference_type='EMPLOYEE_REQUEST'/reference_id=employee_id).

    public function inventoryConsumption(Request $request): void
    {
        $key = $request->param('id');
        $emp = Employee::findByKey($key);
        if (!$emp) Response::error('Employee not found', 404);
        ...
        $topProduct = Database::fetch(
            "SELECT p.name, SUM(m.quantity) AS qty
             FROM inventory_stock_movements m
             ...
             WHERE m.movement_type = 'EMPLOYEE_ISSUE'
               ...
               AND m.reference_id = ?
               AND m.created_at >= (NOW() - INTERVAL 30 DAY)
             GROUP BY m.inv_product_id, p.name
             ORDER BY qty DESC
             LIMIT 1",
            [$employeeId]
        );
        ...
        $history = Database::fetchAll(
            "SELECT m.movement_id, m.inv_product_id, p.name AS product_name, p.sku, p.uom,
                    m.quantity, m.unit_cost, m.total_value, m.remarks, m.created_at
             FROM inventory_stock_movements m
             JOIN inventory_products p ON p.inv_product_id = m.inv_product_id
             WHERE m.movement_type = 'EMPLOYEE_ISSUE'
               AND m.reference_type = 'EMPLOYEE_REQUEST'
        ...
        Response::success([
            ...
            'last_30_days'  => [
                ...
                'most_issued_product' => $topProduct['name'] ?? null,
            ],
            'consumption_history' => array_map(static function (array $row): array {
                return [
