...
class AdminUserController
{
    ...
    public function store(Request $request): void
    {
        ...
        $hasEmail = $rawEmail !== '';
        $email = $hasEmail ? $rawEmail : $phone . '@noemail.inventory.local';
        ...
        $adminEmail = $request->user['email'] ?? 'admin@inventory.com';
        $displayType = ucfirst($userType);
        $headers = "MIME-Version: 1.0\r\nContent-Type: text/html; charset=UTF-8\r\nFrom: Inventory Management System <noreply@inventory.com>\r\n";

        // Email to the dealer (only when a real email was provided)
        if ($hasEmail) {
            $dealerSubject = "Welcome to Inventory Management System – Your Login Credentials";
    ...
    private static function credentialEmailBody(
        ...
        $title = $isAdminCopy ? "New {$userType} Account Created" : "Welcome to Inventory Management System";
        $intro = $isAdminCopy
            ? "A new {$userType} account has been created. Here are the details:"
            : "Your {$userType} account on the Inventory Management System platform is ready. Use the credentials below to log in to the mobile app.";
              ...
              <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
                <tr>
                  <td style="background:#16a34a;padding:28px 36px;">
                    <h1 style="margin:0;color:#ffffff;font-size:22px;">Inventory Management System</h1>
                    <p style="margin:4px 0 0;color:#bbf7d0;font-size:13px;">Inventory & Stamping Management</p>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="background:#f9fafb;padding:16px 36px;border-top:1px solid #e5e7eb;">
                    <p style="margin:0;color:#9ca3af;font-size:12px;">© Inventory Management System · This is an automated message, please do not reply.</p>
                  </td>
                </tr>
              </table>
            </td></tr>
...
HTML;
    ...
    public function update(Request $request): void
    {
        ...
        if ($newPassword !== '') {
            ...
            $hasRealEmail = !str_ends_with($dealerEmail, '@noemail.inventory.local');
            $headers = "MIME-Version: 1.0\r\nContent-Type: text/html; charset=UTF-8\r\nFrom: Inventory Management System <noreply@inventory.com>\r\n";

            // Notify the dealer if they have a real email
            if ($hasRealEmail) {
                $body = self::passwordChangedEmailBody($dealerName, $dealerPhone, $newPassword, false);
                @mail($dealerEmail, 'Your Inventory Management System Login Password Has Been Changed', $body, $headers);
            }
    ...
    private static function passwordChangedEmailBody(
        ...
        $intro = $isAdminCopy
            ...
            : "Your login password on the Inventory Management System platform has been updated by an admin. Use the new credentials below.";
