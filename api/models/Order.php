...
class Order
{
    ...
    /**
     * Create a "direct" sales order that did not originate from the customer
     * portal — e.g. a machine sale converted into an invoice. Such orders have
     * no portal user account (user_id NULL) and carry a free-text customer name.
     * Line items are intentionally omitted: the order total drives revenue, and
     * these sales reference machines/services rather than catalog products.
     *
    ...
    public static function findById(int $orderId): ?array
    {
        $order = Database::fetch(
            'SELECT o.order_id, o.order_number, o.total_amount, o.delivery_fee, o.order_status,
                    ...
                    o.notes, o.source, o.created_at, o.updated_at,
                    u.user_id, COALESCE(o.customer_name, u.name) AS customer_name, u.email, u.phone, u.company_name
             FROM orders o
