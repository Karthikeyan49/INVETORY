...
class DataImportMapper
{
    ...
    private static function commitCustomer(array $m, ?int $actorId): string
    {
        ...
        $email = strtolower($v('email'));
        if ($email === '') {
            $email = preg_replace('/\D+/', '', $phone) . '@import.inventory.local';
        }
        $data = [
            'name' => self::value($m, 'name'),
            'email' => $email,
    ...
    private static function commitOpeningStock(array $m, int $jobId, ?int $actorId): string
    {
        ...
        Database::execute(
            'INSERT INTO stock_items (product_id, location_id, on_hand, avg_cost, updated_at)
             VALUES (?, ?, ?, ?, NOW())
             ON DUPLICATE KEY UPDATE on_hand = VALUES(on_hand), avg_cost = VALUES(avg_cost), updated_at = NOW()',
            [(int)$product['product_id'], (int)$location['location_id'], $quantity, $unitCost]
        );
        Database::execute(
            ...
            [(int)$product['product_id'], (int)$location['location_id'], $quantity, $unitCost, $jobId, self::value($m, 'note') ?: 'Opening stock import', $actorId]
        );
        return 'updated';
    }
    ...
    {
        $row = Database::fetch('SELECT * FROM inventory_locations WHERE LOWER(name) = ? LIMIT 1', [strtolower($name)]);
        if ($row) {
            return $row;
        }
        $id = Database::insert('INSERT INTO inventory_locations (name, is_default, is_active, created_at) VALUES (?, 0, 1, NOW())', [$name]);
        return Database::fetch('SELECT * FROM inventory_locations WHERE location_id = ? LIMIT 1', [$id]);
    }
