...
class PurchaseOrder
{
    ...
    public static function createReceipt(int $poId, array $data, array $lines): int
    {
        ...
        foreach ($lines as $line) {
            ...
            $current = Database::fetch(
                'SELECT quantity, received_qty FROM purchase_order_items WHERE item_id = ? AND po_id = ? FOR UPDATE',
    ...
    public static function voidReceipt(int $grnId, int $actorId, string $reason): void
    {
        ...
        $movements = Database::fetchAll(
            'SELECT * FROM stock_movements WHERE ref_type = "grn" AND ref_id = ? AND direction = "in"',
            [$grnId]
        );
        ...
        foreach ($items as $index => $item) {
            ...
            Database::execute(
                'INSERT INTO purchase_order_items
                    (po_id, description, product_id, hsn_code, quantity, received_qty, unit, unit_price, gst_rate, line_total, sort_order)
                 VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?)',
                [
                    $id,
                    $item['description'],
