...
class User
{
    ...
    public static function create(array $data): int
    {
        ...
        return Database::insert(
            'INSERT INTO users
                (name, email, phone, user_type, company_name, address, city, state, pincode, gst_number, udyam_number, password_hash, is_active, approval_status, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())',
            [
    ...
    /** Whether the tax_view column exists (requirement.txt — Module 5, dual tax login). */
    public static function hasTaxViewColumn(): bool
    {
        static $has = null;
        if ($has !== null) {
            return $has;
        ...
        try {
            $has = Database::fetch("SHOW COLUMNS FROM users LIKE 'tax_view'") !== null;
        } catch (Throwable $e) {
            $has = false;
        }
        return $has;
