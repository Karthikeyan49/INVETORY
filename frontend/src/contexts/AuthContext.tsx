...
const STORAGE_KEY = "eco_admin_auth";

export type AuthRole = "admin" | "dealer";
export type TaxView = "standard" | "extended";
...
interface AuthContextType {
  ...
  userName: string;
  role: AuthRole | null;
  taxView: TaxView;          // Module 5: "extended" login also sees the off-books extra amount
  staffRole: string | null;  // Module 6: "sales" = salesperson
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [taxView, setTaxView] = useState<TaxView>("standard");
  ...
  const applySession = (data: Record<string, string>) => {
    ...
    setTaxView(data.tax_view === "extended" ? "extended" : "standard");
    ...
    try {
      ...
      if (response?.success && type === "admin") { // dealer accounts removed — staff only
        const session = {
          ...
          tax_view: response.data.user.tax_view === "extended" ? "extended" : "standard",
          staff_role: response.data.user.staff_role || "",
  ...
  return (
    <AuthContext.Provider value={{ isAuthenticated, loading, adminEmail, userName, role, taxView, staffRole, login, logout }}>
      {children}
