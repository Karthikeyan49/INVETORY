...
// ── Create manual / walk-in order (admin) ─────────────────────────────────
...
export async function createManualOrder(input: CreateManualOrderInput): Promise<ApiOrder> {
  const res = await apiFetch<{ data: ApiOrder }>("/admin/orders", {
    method: "POST",
...
export async function updateOrderStatus(
  id: number,
  status: string,
  extra?: { tracking_number?: string; cancel_reason?: string }
): Promise<{ invoice_generated?: boolean; invoice_number?: string }> {
