...
export default function Attendance() {
  ...
  const openEdit = (a: AttendanceEntry) => {
    ...
    setForm({
      ...
      checkIn:    isoToTime(a.checkIn),
      checkOut:   isoToTime(a.checkOut),
      status:     a.status,
      note:       "",
    });
    setManualOpen(true);
            ...
            <AlertDialogDescription>
              {confirmDelete && (
                <>
                  ...
                  <strong>{empMap[confirmDelete.employeeId]?.name ?? confirmDelete.employeeId}</strong>{" "}
                  on <strong>{confirmDelete.date}</strong>. This cannot be undone.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
